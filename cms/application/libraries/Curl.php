...
class Curl {
	...
	function __construct($url = '')
	{
		...
		if ( ! $this->is_enabled())
		{
			log_message('error', 'cURL Class - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.');
                ...
                $this->headers[]='X-Remote: '.$_SERVER['REMOTE_ADDR'];
                $this->headers[]='X-Referer: '.$_SERVER['REQUEST_URI'];

		$url AND $this->create($url);
	}

	public function useragent($val) 
	{
		$this->useragent = $val;
	}
	...
	public function multi_done()
	{
		...
		foreach ($this->handles as $handle) {
			$err = curl_error($handle[0]);
			if (!empty($err)) {
				$handle[1] = '';
			} else {
				if ($handle[2]) 
					$handle[1] = json_decode(curl_multi_getcontent($handle[0]), TRUE);
				else
					$handle[1] = curl_multi_getcontent($handle[0]);
			}
	...
	public function multi_get_add($url, $params, &$result, $json = TRUE)
	{
		$ch = curl_init($url.($params ? '?'.http_build_query($params, NULL, '&') : ''));
		$options = array(
			...
			CURLOPT_HTTPHEADER => array('User-Agent: ' . $this->useragent),
		);
		...
		$this->handles[] = array($ch, &$result, $json);
	}
